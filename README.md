...
On first launch the app asks you to pick a folder for your files, then creates two
demo files (`Welcome.md`, `HelloWorld.kt`) in it so every feature can be demonstrated
immediately.
...
│   ├── data/
│   │   ├── FileRepository.kt        ← read/write files in the folder the user picked
...
│   ├── files/
│   │   └── FilesActivity.kt         ← full-screen list of every saved file
│   └── versions/
...
| AndroidX DocumentFile 1.0.1 | reading/writing files in the folder the user picks (Storage Access Framework) |
...
### Editor engine & UI
- **New / Open / Save / Save As / Recent files** - on first launch the app asks the
  user to pick a folder (the system folder picker / Storage Access Framework);
  `FileRepository` then reads and writes every file there instead of the app's
  private storage, so no special permission is needed and the files stay visible
  outside the app too. The sidebar (navigation drawer) lists recent files
  (`RecentFilesStore`); *Open Files* opens a full-screen list of every saved file
  (`FilesActivity`), which also has a menu action to open that exact folder in the
  device's file manager. *Save As* offers an **encoding** choice (UTF-8 / UTF-16 /
  ISO-8859-1); the chosen encoding is remembered per file in the database and the
  spinner now pre-selects the file's current encoding when re-saving.
...
- **Read-only flag** - menu -> *Read-only* removes the EditText's key listener
  (typing blocked) and blocks Cut, Paste, Save, Replace, Format, Restore, Undo,
  Redo, and Create-snapshot; Copy still works since it doesn't change the text.
  The lock persists in the `tracked_files` table.
